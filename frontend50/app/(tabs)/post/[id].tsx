...
export default function PostDetail() {
  ...
  const handleReply = async () => {
    ...
    try {
      ...
      setReplyText(""); setReplyingTo(null); fetchPost();
  ...
  const handleDelete = async () => {
    Alert.alert("Delete Post", "Are you sure? This cannot be undone.", [
      ...
      { text: "Delete", style: "destructive", onPress: async () => {
        try { await api.delete(`/api/posts/${id}`); router.push("/(tabs)/feed"); }
        catch { Alert.alert("Error", "Could not delete post. Try again."); }
      }},
    ]);
  };

  const handleEdit = () => { setEditContent(post?.content || ""); setEditModalVisible(true); };
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      setEditModalVisible(false); fetchPost();
      Alert.alert("✅ Post updated!");
    } catch { Alert.alert("Error", "Could not update post. Try again."); }
  ...
  const submitReport = async (reason: string) => {
    try {
      ...
      Alert.alert(err?.response?.status === 400 ? "Already Reported" : "Error",
  ...
  const getReplies = (commentId: number) => post?.comments?.filter((c: any) => c.parentId === commentId) || [];
    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: colors.background }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      ...
      <Modal visible={editModalVisible} transparent animationType="slide" onRequestClose={() => setEditModalVisible(false)}>
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
          ...
          <ScrollView style={{ backgroundColor: colors.card, borderTopLeftRadius: 20, borderTopRightRadius: 20 }} contentContainerStyle={{ padding: 20 }} keyboardShouldPersistTaps="handled">
            <Text style={{ color: colors.text, fontSize: 20, fontWeight: "900", marginBottom: 16 }}>✏️ Edit Post</Text>
            <TextInput
              value={editContent} onChangeText={setEditContent} multiline autoFocus
              placeholder="What's on your mind?" placeholderTextColor={colors.textMuted}
              style={{ backgroundColor: colors.background, color: colors.text, paddingHorizontal: 16, paddingVertical: 12, borderRadius: 12, borderWidth: 1, borderColor: colors.blue, fontSize: 15, lineHeight: 22, minHeight: 120, textAlignVertical: "top", marginBottom: 16 }}
            />
            <View style={{ flexDirection: "row", gap: 10, marginBottom: 30 }}>
              <TouchableOpacity onPress={() => setEditModalVisible(false)} style={{ flex: 1, backgroundColor: colors.border, padding: 14, borderRadius: 12, alignItems: "center" }}>
                <Text style={{ color: colors.text, fontWeight: "700" }}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={handleSaveEdit} disabled={saving || !editContent.trim()} style={{ flex: 1, backgroundColor: saving || !editContent.trim() ? colors.border : colors.blue, padding: 14, borderRadius: 12, alignItems: "center" }}>
      ...
      <View style={{ paddingTop: 60, paddingHorizontal: 20, paddingBottom: 16, borderBottomWidth: 1, borderBottomColor: colors.border, flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
        ...
        {isMyPost ? (
          <View style={{ flexDirection: "row", gap: 8 }}>
            <TouchableOpacity onPress={handleEdit} style={{ backgroundColor: colors.card, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 10, borderWidth: 1, borderColor: colors.blue + "44" }}>
              <Text style={{ color: colors.blue, fontSize: 13, fontWeight: "700" }}>✏️ Edit</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={handleDelete} style={{ backgroundColor: colors.card, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 10, borderWidth: 1, borderColor: "#ef444444" }}>
              <Text style={{ color: "#ef4444", fontSize: 13, fontWeight: "700" }}>🗑️ Delete</Text>
            </TouchableOpacity>
          </View>
        ) : (
          <TouchableOpacity onPress={handleReport} style={{ backgroundColor: colors.card, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 10, borderWidth: 1, borderColor: colors.border }}>
            <Text style={{ color: colors.textSecondary, fontSize: 13, fontWeight: "700" }}>🚩 Report</Text>
          </TouchableOpacity>
        )}
      </View>
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View>
            <View style={{ backgroundColor: colors.card, margin: 16, borderRadius: 16, borderWidth: 1, borderColor: colors.border, padding: 16 }}>
              {/* POST TYPE BADGE */}
              <View style={{ alignSelf: "flex-start", backgroundColor: post?.postType === "QUESTION" ? "#1e3a8a" : post?.postType === "SERVICE" ? "#78350f" : post?.postType === "BEFORE_AFTER" ? "#991b1b" : "#5b21b6", paddingHorizontal: 10, paddingVertical: 3, borderRadius: 10, marginBottom: 12 }}>
              ...
              <TouchableOpacity onPress={() => router.push(`/(tabs)/user/${post?.user?.id}`)} style={{ flexDirection: "row", alignItems: "center", marginBottom: 12, gap: 10 }}>
                <View style={{ width: 44, height: 44, borderRadius: 22, backgroundColor: colors.border, justifyContent: "center", alignItems: "center", borderWidth: 2, borderColor: post?.user?.role === "MECHANIC" ? colors.blue : colors.green, overflow: "hidden" }}>
                  {post?.user?.profilePhoto ? (
                    <Image source={{ uri: post.user.profilePhoto }} style={{ width: 44, height: 44, borderRadius: 22 }} />
                  ) : (
                    <Text style={{ color: colors.text, fontWeight: "700", fontSize: 18 }}>{post?.user?.name?.[0]?.toUpperCase() || "?"}</Text>
                  )}
                </View>
                <View>
                  <View style={{ flex: 1, flexDirection: "row", alignItems: "center", flexWrap: "wrap", gap: 6 }}>
                    <Text style={{ color: colors.text, fontWeight: "700", fontSize: 15 }}>{post?.user?.name || "Anonymous"}</Text>
                    {post?.user?.role === "MECHANIC" && post?.user?.isVerified ? (
                      <View style={{ paddingHorizontal: 8, paddingVertical: 3, borderRadius: 8, borderWidth: 1, borderColor: "#60a5fa", backgroundColor: "#1e3a8a" }}>
                        <Text style={{ color: "white", fontSize: 10, fontWeight: "700" }}>🏁 Verified Mechanic</Text>
                      </View>
                    ) : (
                      <View style={{ paddingHorizontal: 8, paddingVertical: 3, borderRadius: 8, backgroundColor: post?.user?.role === "MECHANIC" ? "#1e3a8a" : "#064e3b", borderWidth: 1, borderColor: post?.user?.role === "MECHANIC" ? colors.blue : colors.green }}>
                        <Text style={{ color: "white", fontSize: 10, fontWeight: "700" }}>{post?.user?.role === "MECHANIC" ? "🏁 Mechanic" : "🔧 DIYer"}</Text>
                      </View>
                    )}
                  </View>
                </View>
              ...
              <View style={{ flexDirection: "row", alignItems: "center", gap: 6, paddingTop: 12, borderTopWidth: 1, borderTopColor: colors.border }}>
                ...
                <Text style={{ color: colors.textSecondary, fontSize: 14 }}>{post?.likes?.length || 0} likes</Text>
                <Text style={{ color: colors.textSecondary, fontSize: 14, marginLeft: 12 }}>💬 {post?.comments?.length || 0} comments</Text>
              </View>
            </View>

            <Text style={{ color: colors.textSecondary, fontSize: 14, paddingHorizontal: 20, marginBottom: 8 }}>
        ...
        ListEmptyComponent={
          <View style={{ alignItems: "center", marginTop: 40 }}>
            <Text style={{ color: colors.textMuted, fontSize: 15 }}>No comments yet. Be the first!</Text>
          </View>
        }
        renderItem={({ item }) => {
            ...
            <View style={{ marginHorizontal: 16, marginBottom: 10 }}>
              <View style={{ backgroundColor: colors.card, borderRadius: 12, borderWidth: 1, borderColor: colors.border, padding: 14 }}>
                <TouchableOpacity onPress={() => router.push(`/(tabs)/user/${item.user?.id}`)} style={{ flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 6 }}>
                  <View style={{ width: 28, height: 28, borderRadius: 14, backgroundColor: colors.border, justifyContent: "center", alignItems: "center", borderWidth: 1, borderColor: item.user?.role === "MECHANIC" ? colors.blue : colors.green }}>
                    <Text style={{ color: colors.text, fontSize: 12, fontWeight: "700" }}>{item.user?.name?.[0]?.toUpperCase() || "?"}</Text>
                  </View>
              ...
              {isReplying && (
                <View style={{ marginLeft: 24, marginTop: 6, flexDirection: "row", gap: 8, alignItems: "center" }}>
                  <TextInput
                    ...
                    placeholder={`Reply to ${item.user?.name}...`} placeholderTextColor={colors.textMuted}
                    style={{ flex: 1, backgroundColor: colors.background, color: colors.text, paddingHorizontal: 14, paddingVertical: 8, borderRadius: 16, borderWidth: 1, borderColor: colors.blue + "44", fontSize: 14 }}
                  />
                  <TouchableOpacity onPress={handleReply} disabled={submittingReply || !replyText.trim()} style={{ backgroundColor: submittingReply || !replyText.trim() ? colors.border : colors.blue, paddingHorizontal: 14, paddingVertical: 8, borderRadius: 16 }}>
                    <Text style={{ color: "white", fontWeight: "700", fontSize: 13 }}>{submittingReply ? "..." : "Send"}</Text>
                  </TouchableOpacity>
                </View>
              )}
            </View>
      ...
      {/* MAIN COMMENT INPUT */}
      <View style={{ flexDirection: "row", padding: 16, borderTopWidth: 1, borderTopColor: colors.border, gap: 10, alignItems: "center" }}>
        <TextInput
          value={comment} onChangeText={setComment} placeholder="Add a comment..."
          placeholderTextColor={colors.textMuted}
          style={{ flex: 1, backgroundColor: colors.card, color: colors.text, paddingHorizontal: 16, paddingVertical: 10, borderRadius: 20, borderWidth: 1, borderColor: colors.border }}
        />
        <TouchableOpacity onPress={handleComment} disabled={submitting || !comment.trim()} style={{ backgroundColor: submitting || !comment.trim() ? colors.border : colors.blue, paddingHorizontal: 16, paddingVertical: 10, borderRadius: 20 }}>
